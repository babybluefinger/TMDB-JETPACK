...
class OnTheAirTVAdapter : PagedListAdapter<TvShowEntity, OnTheAirTVAdapter.ViewHolder>(DIFF_CALLBACK) {
    ...
    class ViewHolder(private val binding: ListNewMovieBinding) : RecyclerView.ViewHolder(binding.root){
        fun bind(tvShowsEntity: TvShowEntity) {
            ...
            binding.apply {
                Glide.with(itemView.context).load("https://image.tmdb.org/t/p/w500/${tvShowsEntity.poster_path}").into(ivNewMovie)
                tvTitle.text = tvShowsEntity.name
